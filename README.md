...
> **Mission：探索如何最大化 agent 的潜力**——规则只是六层杠杆之一（防下限的那层）。完整杠杆全景与路线：`docs/potential-roadmap.md`（能力基线/环境工程/知识复利/反馈回路/任务设计/协作结构）。核心洞察：潜力是被"解锁"的不是被"激发"的，探索半径 ∝ 1/行动摩擦。
> 下面是其中"规则+技能"层的能力体系（可信度地基，已冻结不再加）：
...
- **COMMAND（7 个 slash command）**：手动 `/` 显式调用的薄命令层——3 个 handoff（跨 session 接力）+ 2 个 KB 别名 + **/task-brief**（高预算任务开工简报，判据先红后绿+停止条件）+ **/cross-check**（高风险结论 fresh-context 交叉证伪/抽检）。
...
bash setup.sh        # 全局装：宪法 + 10 skill + 7 slash command，单一真源在 ~/.ai-coding-pack
```
