...
4. **抽检制度（校准——用户不知道答案对不对，KB 的可信度必须自证）**：
   - 每个维护周期，从台账里抽 **≥3 条（或 20%，取大者）**已交付答案，用 **fresh context**（不带原答案的独立 agent/session）重走证伪：查上游准入、查最终结果字段、找反面证据。结论回填台账"答准了吗"列。
   - **错误率写进 CHANGELOG**（如"本期抽检 5 条：4 对 1 错，错因=中游证据虚标"），让业务方看得见该多信这个 KB。
   - **熔断**：一个周期内出现任何一条**标了 `[可直接决策]` 却被抽检推翻**的答案 → 全库 `[可直接决策]` 标签暂停使用（一律降级为"决策前必须核验"），直到定位虚标原因、修完规则并在下期抽检清零。虚标可决策性是最高级违规。
